...
class PricingRuleCRUD(BaseCRUD[PricingRules]):
    ...
    async def search_pricing_rules(
        ...
        if component_name:
            query = query.where(self.model.component_name.ilike(f"%{component_name}%"))
        if component_type:
            query = query.where(self.model.component_type == component_type)
...
class CustomerPricingCRUD(BaseCRUD[CustomerPricing]):
    def __init__(self):
        super().__init__(CustomerPricing)
